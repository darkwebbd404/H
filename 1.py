...
class FF_CLIENT(threading.Thread):
    ...
    def GenResponsMsg(self, Msg):
        ...
        1: 1,
        2: {
            1: 11594026651,#bot account id
            2: 3075740690,#clan id
            3: 1,
            4: str(Msg),
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
مرحبًا بكم في كلان [00ff00]CODEX HAT[ffffff]!

[C][B]
إذا أردت معرفة الأوامر، فقط أرسل: [00ff00]🗿/🗿help🗿[ffffff]

[C][B]
تنبيه! البوت لا يشتغل إلا لمن لديهم تاغ الكلان: [00ff00]CDX[ffffff]
"""
                            )
                        )
                else:
...
####################################
            if "1200" in data.hex()[0:4] and b"/help" in data: 
	                ...
	                sender_name = parsed_data['5']['data']['9']['data']['1']['data']
	                response_message = f"""
test by fox, I am the king, I know⁦(⁠^⁠^⁠)⁩
"""
	                clients.send(self.GenResponsMsg(response_message))
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4035300235", password="A777C3E0BBBA43B3BD52CDFD4C31128A6E0247DD1808D8A7E55EEE49D64C3315")
        client_thread.start()
    except Exception as e:
